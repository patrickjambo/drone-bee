...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FE] flex font-sans text-gray-800">
      {/* Sidebar */}
      <AdminSidebar unwatchedAlerts={stats.unreadAlertsCount} />
      ...
      <main className="ml-[280px] flex-1 p-8 overflow-y-auto">
        ...
        <header className="flex justify-between items-end mb-8">
          <div>
            <h1 className="text-3xl font-bold text-[#2B3674] mb-1">Dashboard Overview</h1>
            <p className="text-[#A3AED0] font-medium">{currentDate}</p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-[#1E2336] to-[#2B3674] rounded-[20px] p-6 shadow-sm relative overflow-hidden text-white">
            <div className="relative z-10">
              <p className="text-sm font-medium text-[#A3AED0] mb-1">TODAY\\'S EARNINGS</p>
              <h2 className="text-3xl font-bold mb-6">RWF {stats.totalRevenue.toLocaleString()}</h2>
              <div className="space-y-1">
                <p className="text-sm text-[#A3AED0]">Total Sales: <span className="text-white font-medium">{stats.unitsSold}</span></p>
                <p className="text-sm text-[#A3AED0]">Avg Per Sale: <span className="text-white font-medium">RWF {stats.unitsSold > 0 ? Math.round(stats.totalRevenue / stats.unitsSold).toLocaleString() : 0}</span></p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E9EDF7] relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4">
              <div className="w-10 h-10 rounded-full bg-[#E5ECF6] flex items-center justify-center">
                <Users className="text-[#4318FF] w-5 h-5" />
              </div>
            </div>
            <p className="text-sm font-medium text-[#A3AED0] mb-1">ACTIVE AGENTS</p>
            <h2 className="text-3xl font-bold text-[#2B3674] mb-6">{stats.activeManagers}</h2>
            <div className="flex items-center text-sm font-medium text-[#01B574]">
              ...
              <span className="text-[#A3AED0]">registered users</span>
            </div>
            {/* Simple Line Graphic */}
          ...
          <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E9EDF7] relative">
            ...
            <p className="text-sm font-medium text-[#A3AED0] mb-1">TOTAL PRODUCTS</p>
            <h2 className="text-3xl font-bold text-[#2B3674] mb-4">{products.length}</h2>
            <div className="space-y-1">
              <p className="text-sm text-[#A3AED0]"><span className="text-[#2B3674] font-medium border-l-2 border-[#01B574] pl-2">{rawHoneyCount}</span> X Raw Honey</p>
              <p className="text-sm text-[#A3AED0]"><span className="text-[#2B3674] font-medium border-l-2 border-[#EE5D50] pl-2">{accessoriesCount}</span> X Accessories</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E9EDF7] relative">
            ...
            <p className="text-sm font-medium text-[#A3AED0] mb-1">UNRESOLVED ALERTS</p>
            <h2 className="text-3xl font-bold text-[#2B3674] mb-4">{stats.unreadAlertsCount}</h2>
            <div className="space-y-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-[#E9EDF7] rounded-[20px] p-8 flex items-center justify-between shadow-inner">
            <div>
              <p className="text-sm font-bold text-[#A3AED0] tracking-wider mb-2">PRODUCTS AVAILABLE IN STORE</p>
              <div className="flex items-baseline gap-2">
                <span className="text-6xl font-black text-[#2B3674]">{totalStockInStore}</span>
                <span className="text-xl font-bold text-[#2B3674]">in stock</span>
              </div>
            </div>
            <div className="text-right space-y-2">
              {products.slice(0, 5).map(p => (
                 <p key={p.id} className="text-[#2B3674] font-medium"><span className="bg-white text-xs px-2 py-1 rounded shadow-sm mr-2">{p.stock_units}</span> {p.name}</p>
              ))}
            </div>
          </div>

          <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E9EDF7] flex flex-col justify-center items-center">
            <h3 className="text-lg font-bold text-[#2B3674] w-full text-left mb-6">TODAY\\'S SALES</h3>
            <div className="text-center w-full">
               <span className="text-5xl font-black text-[#2B3674]">{stats.unitsSold}</span>
               <p className="text-[#A3AED0] font-medium mt-2 mb-4">transactions completed</p>
               {/* Chart placeholder */}
               <div className="w-full h-24 bg-[#F4F7FE] rounded-lg mt-4 flex items-end justify-between px-4 pb-2">
                 {[40, 20, 60, 30, 80, 50, 90, 45, 60, 30, 20, 50].map((h, i) => (
                   <div key={i} className="w-[6%] bg-[#4318FF] rounded-t-sm opacity-80" style={{ height: \`\${h}%\` }}></div>
                 ))}
               </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E9EDF7]">
               <h3 className="text-lg font-bold text-[#2B3674] mb-4">QUICK REPORT GENERATION</h3>
               <button onClick={handleDownloadReport} className="w-full bg-[#4318FF] hover:bg-[#3211b8] text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors shadow-lg shadow-[#4318FF]/20">
                 <Download size={20} />
            ...
            {/* Create Sales Agent */}
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E9EDF7]">
               <h3 className="text-lg font-bold text-[#2B3674] mb-6">CREATE SALES AGENT QUICK</h3>
               <div className="space-y-4">
                 <div>
                   <label className="text-sm font-medium text-[#2B3674] mb-2 block">Agent Username</label>
                   <input 
                     value={agentName}
                     onChange={e => setAgentName(e.target.value)}
                     className="w-full bg-[#F4F7FE] border border-[#E9EDF7] rounded-xl px-4 py-3 text-[#2B3674] focus:outline-none focus:border-[#4318FF]"
                     placeholder="no spaces"
                   />
                 </div>
                 <div>
                   <label className="text-sm font-medium text-[#2B3674] mb-2 block">Temporary Password (optional)</label>
                   <input 
                     type="password"
                     value={agentPass}
          ...
          <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E9EDF7] relative overflow-hidden">
             ...
             <h3 className="text-lg font-bold text-[#2B3674] mb-6">QUICK ADD PRODUCTS STOCK</h3>
             
             <div className="p-6 bg-white border border-[#E9EDF7] shadow-xl rounded-[20px] mb-8 relative z-10">
               <div className="space-y-4 mb-6">
                 <div>
                   <label className="text-sm font-medium text-[#A3AED0] mb-2 block">Select Product to Restock</label>
                   <select 
                      value={selectedProduct} 
                      onChange={e => setSelectedProduct(e.target.value)}
                      className="w-full bg-[#F4F7FE] border-none rounded-xl px-4 py-3 text-[#2B3674] font-medium appearance-none outline-none"
                   >
             ...
             <div className="mt-8 relative z-10">
               <h4 className="text-sm font-bold text-[#A3AED0] mb-4 uppercase">Recent Inventory List</h4>
                 ...
                 {products.map(p => (
                    <div key={p.id} className="flex items-center justify-between p-3 hover:bg-[#F4F7FE] rounded-xl transition-colors bg-white border border-transparent hover:border-[#E9EDF7]">
                      <div className="flex items-center gap-3">
                        <div className={\`w-10 h-10 rounded-lg flex items-center justify-center font-bold \${p.stock_units < 10 ? 'bg-[#FFE2E5] text-[#EE5D50]' : 'bg-[#E5ECF6] text-[#4318FF]'}\`}>
                           {p.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-bold text-[#2B3674]">{p.name}</p>
                          <p className={\`text-sm \${p.stock_units < 10 ? 'text-[#EE5D50] font-bold' : 'text-[#A3AED0]'}\`}>{p.stock_units} In Stock</p>
                        </div>
                      </div>
                      <span className="font-bold text-[#2B3674]">RWF {p.price_per_unit.toLocaleString()}</span>
                    </div>
                 ))}
                 {products.length === 0 && <p className="text-[#A3AED0] text-sm py-4">No products found.</p>}
               </div>
             </div>
